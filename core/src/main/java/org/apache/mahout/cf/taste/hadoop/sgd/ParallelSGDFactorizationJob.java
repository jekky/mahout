...
/**
 * <p>
 * MapReduce implementation of the parallel SGD factorization algorithm
 * described in
 * 
 * <p>
 ...
 * </ol>
 */
public class ParallelSGDFactorizationJob extends AbstractJob {

	private static final Logger log = LoggerFactory
			.getLogger(ParallelSGDFactorizationJob.class);

	static final String NUM_FEATURES = ParallelSGDFactorizationJob.class
			.getName() + ".numFeatures";
	static final String LAMBDA = ParallelSGDFactorizationJob.class.getName()
			+ ".lambda";
	static final String ALPHA = ParallelSGDFactorizationJob.class.getName()
			+ ".alpha";
	static final String NUM_ENTITIES = ParallelSGDFactorizationJob.class
			.getName() + ".numEntities";

	static final String USES_LONG_IDS = ParallelSGDFactorizationJob.class
			.getName() + ".usesLongIDs";
	static final String TOKEN_POS = ParallelSGDFactorizationJob.class.getName()
			+ ".tokenPos";

	private int numIterations;
	private int numIndependentBlocks;
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new ParallelSGDFactorizationJob(), args);
	}
	...
	public int run(String[] args) throws Exception {
		...
		addOption("numIndependentBlocks", null,
				"number of independent blocks per group", true);
		...
		numIndependentBlocks = Integer.parseInt(getOption("numIndependentBlocks"));
		...
		// TODO this could be fiddled into one of the upper jobs
		Job averageUserRatings = prepareJob(pathToUserRatings(),
				getTempPath("averageUserRatings"), AverageRatingMapper.class,
				IntWritable.class, VectorWritable.class,
				MergeVectorsReducer.class, IntWritable.class,
				VectorWritable.class);
		averageUserRatings.setCombinerClass(MergeVectorsCombiner.class);
		succeeded = averageUserRatings.waitForCompletion(true);
		if (!succeeded) {
			return -1;
		}

		Vector averageItemRatingVector = ParallelSGDUtils.readFirstRow(
				getTempPath("averageItemRatings"), getConf());
		Vector averageUserRatingVector = ParallelSGDUtils.readFirstRow(
				getTempPath("averageUserRatings"), getConf());

		numItems = averageItemRatingVector.getNumNondefaultElements();
		numUsers = averageUserRatingVector.getNumNondefaultElements();

		log.info("Found {} users and {} items", numUsers, numItems);

		/* create an initial M */
		initializeMorU(averageItemRatingVector, pathM);
		initializeMorU(averageUserRatingVector, pathU);

		/* Step 2: iteratively update M and U */
		// Iterate over {numIterations} epoch
		for (int currentIteration = 0; currentIteration < numIterations; currentIteration++) {
			ArrayList<List> patterns = ParallelSGDUtils.getIndependentGroups(numIndependentBlocks);
			
			for (List pattern : patterns) {
				// prepare the sgd job to update
				/* 1. prepare input paths to specific parts */
	...
	private void initializeMorU(Vector averageRatings) throws IOException {
		...
		try {
			writer = new SequenceFile.Writer(fs, getConf(), new Path(
					pathToM(-1), "part-m-00000"), IntWritable.class,
					VectorWritable.class);
